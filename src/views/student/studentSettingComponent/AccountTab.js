...
function AccountTab() {
  const [setting, setSetting] = useState({
    first_name: "", last_name: "", title: "", biography: '', Facebook_Link: "", Twitter_Link: "", LinkedIn_Link: "", Youtube_Link: ""
  })
  ...
  const onSubmit = async () => {
    ...
    await axios.post(`${BaseUrl}foretend/acountinfo`, urlencoded, {
      headers: {
        "auth": localStorage.getItem("LMS_Token")
      },
    }).then((response) => {
      setSetting(response.data)
      history.push("/student/student_dashboard")
    }).catch((error) => {
      if (error.response) {
        confirmAlert({
          customUI: ({ onClose }) => {
